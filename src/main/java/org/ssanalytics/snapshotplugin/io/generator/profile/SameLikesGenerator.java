...
public class SameLikesGenerator extends AbstractSameCategoryGenerator implements ProfileGenerator{

    public static String NAME = "Who Likes the Same Graph";
    ...
    protected Map<ICategorizedDomain, List<NamedItem>> getCategoryMap() {
        try {
            ...
            for (IProfile profile : this.profiles) {
                catMap.putAll(DaoFactory.getLikeDAO().getWhoLikesTheSameAsProfileForSnapshotLatestVersion(snapshot.getValue(), profile.getId()));
            }
            return catMap;
        } catch (Exception ex) {
