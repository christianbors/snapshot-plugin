 ...
 * @author chw
 */
public class LocationPostFetcher {

    private LocationPostFetcher() {
    }
    private static LocationPostFetcher instance = null;

    public static LocationPostFetcher getInstance() {
        if (instance == null) {
            instance = new LocationPostFetcher();
        }
        return instance;
    }
    ...
    public Map<ISharedPlace, List<NamedItem>> getWhoHasBeenWhereSnapshotLatestVersion(String snapshot) throws Exception {
        ...
        for (IFriend friend : friendList) {
            ...
            for (ISharedPlace place : DaoFactory.getPostDAO().getLocationsForProfileSnapshotLatestVersion(friend.getId(), snapshot)) {
