...
import org.ssanalytics.snapshotplugin.io.generator.profile.ProfileGenerator;
 ...
 * @author christian
 */
public class ProfileGeneratorComboBoxModel implements ComboBoxModel<ComboItemGenerator> {

    private GeneratorController genController = Lookup.getDefault().lookup(GeneratorController.class);
    ...
    public ProfileGeneratorComboBoxModel() {
        if (genController != null) {
            ...
            for (Generator gen : Lookup.getDefault().lookupAll(ProfileGenerator.class)) {
                ComboItemGenerator genItem = new ComboItemGenerator(gen);
                generators.add(genItem);
            }
        }
